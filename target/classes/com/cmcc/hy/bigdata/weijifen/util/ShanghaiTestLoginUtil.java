...
public class ShanghaiTestLoginUtil {

	private static final Logger LOG = LoggerFactory.getLogger(ShanghaiTestLoginUtil.class);
	...
	public static Configuration getConf(Configuration conf)
	{
		...
		System.setProperty("java.security.krb5.conf", "/opt/huawei/krb5.conf");
		conf.set("username.client.keytab.file", "/opt/client/user.keytab");
		conf.set("username.client.kerberos.principal", "hadoop@HADOOP.COM");
		setJaasConf("Client", "hadoop@HADOOP.COM", "/opt/client/user.keytab");
		try {
		...
		LOG.info("hbase.zookeeper.property.clientPort:" + conf.get("hbase.zookeeper.property.clientPort"));
		LOG.info("hbase.zookeeper.quorum: " + conf.get("hbase.zookeeper.quorum"));
		LOG.info("zookeeper.server.principal:" + System.getProperty("zookeeper.server.principal"));
		LOG.info("java.security.auth.login.config:" + System.getProperty("java.security.auth.login.config"));
		LOG.info("zookeeper.sasl.clientconfig:" + System.getProperty("zookeeper.sasl.clientconfig"));
		LOG.info("javax.security.auth.login.Configuration:" + javax.security.auth.login.Configuration.getConfiguration());
		LOG.info("javax.security.auth.login.Configuration Client Entry:" + javax.security.auth.login.Configuration.getConfiguration().getAppConfigurationEntry("Client"));
